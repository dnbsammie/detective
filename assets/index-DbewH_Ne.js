(function(){const n=document.createElement("link").relList;if(n&&n.supports&&n.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))o(t);new MutationObserver(t=>{for(const i of t)if(i.type==="childList")for(const s of i.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&o(s)}).observe(document,{childList:!0,subtree:!0});function a(t){const i={};return t.integrity&&(i.integrity=t.integrity),t.referrerPolicy&&(i.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?i.credentials="include":t.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function o(t){if(t.ep)return;t.ep=!0;const i=a(t);fetch(t.href,i)}})();function R(){const e=document.createElement("main"),n=document.createElement("div");return n.className="container",e.appendChild(n),e}const x="/detective/assets/img_2-xb6EFWwk.webp";function P(){const e=document.createElement("section");return e.id="hero",e.setAttribute("aria-labelledby","hero-title"),e.innerHTML=`
  ...
  <div class="hero-r">
          <img src="${x}" alt="player img">
  </div>
  `,e.querySelector("#goToHistory")?.addEventListener("click",()=>{document.getElementById("history")?.scrollIntoView({behavior:"smooth"})}),e.querySelector("#goToRules")?.addEventListener("click",()=>{document.getElementById("rules")?.scrollIntoView({behavior:"smooth"})}),e}function D(){const e=document.createElement("section");return e.id="history",e.setAttribute("aria-labelledby","history-title"),e.innerHTML=`
...
<button id="goToRules" aria-label="Ir a las reglas">Ir a las reglas del juego</button>
  ...
  `,e.querySelector("#goToRules")?.addEventListener("click",()=>{document.getElementById("rules")?.scrollIntoView({behavior:"smooth"})}),e}function H(){const e=document.createElement("section");return e.id="rules",e.setAttribute("aria-labelledby","rules-title"),e.innerHTML=`
  ...
  `,e.querySelector("#startGame")?.addEventListener("click",()=>{T("game")}),e}function N(){const e=R(),n=e.querySelector(".container");if(!n)throw new Error("Container no encontrado");return n.appendChild(P()),n.appendChild(D()),n.appendChild(H()),e}function z(){const e=document.createElement("main"),n=document.createElement("div");return n.className="container",e.appendChild(n),e}function O(e=[],n=[]){const a=document.createElement("div");a.className="notebook",a.innerHTML=`
    <h6>NOTAS</h6>
    <hr>
    <div class="toggle">
  ...
  `;const o=a.querySelector(".nb-notes"),t=a.querySelector(".nb-suspects");return e.forEach(i=>{const s=document.createElement("li");s.textContent=i,o.appendChild(s)}),n.forEach(i=>{const s=document.createElement("li");s.textContent=i,t.appendChild(s)}),a.querySelector("#nb-notes")?.addEventListener("click",()=>{o.style.display="block",t.style.display="none"}),a.querySelector("#nb-suspects")?.addEventListener("click",()=>{o.style.display="none",t.style.display="block"}),a}const _=4;function $(){const e=document.createElement("div");e.className="gamelog",e.innerHTML=`
    <h6>GAME LOG</h6>
    <hr>
    <ul class="log-list"></ul>
  `;const n=e.querySelector(".log-list");function a(o){const t=document.createElement("li");if(t.textContent=o,n.appendChild(t),n.children.length>_){const i=n.firstChild;i&&n.removeChild(i)}e.scrollTop=e.scrollHeight}return{element:e,add:a}}function F(e,n,a,o){const t=document.createElement("div");t.className="panel",t.innerHTML=`
        <div class="panel-options"></div>
    `;const i=t.querySelector(".panel-options");return n.forEach((s,l)=>{const r=document.createElement("div");r.className="option",r.innerHTML=`<p>${s}</p>`,r.addEventListener("click",()=>{const f=l===a;o(l),r.classList.add(f?"correct":"wrong"),setTimeout(()=>{t.dispatchEvent(new CustomEvent("panelFinished",{detail:{correct:f}}))},700)}),i.appendChild(r)}),t}function j(e=[]){const n=document.createElement("div");n.className="inventory",n.innerHTML=`
    <h6>INVENTARIO</h6>
    <hr>
  ...
  `,e.querySelector("#goHome")?.addEventListener("click",n=>{n.preventDefault(),C(),q("index.html")}),e}function A(){const e=z(),n=e.querySelector(".container");if(!n)throw new Error("Container no encontrado");n.innerHTML="";const a=re();n.appendChild(a);const o=L(),t=h[o],i=t.item,s=n.querySelector(".notebook"),l=n.querySelector(".gamelog"),r=n.querySelector(".content");let c=n.querySelector(".gamePanel");if(!c){const d=n.querySelector(".game-m");if(d){c=document.createElement("div"),c.className="gamePanel";const u=d.querySelector(".content");u&&u.nextSibling?d.insertBefore(c,u.nextSibling):d.appendChild(c)}}const v=n.querySelector(".inventory");if(s){const d=O(t.notes,t.suspects);s.appendChild(d)}if(r&&(r.innerHTML=`
            <p class="context"><strong>Contexto:</strong> ${t.context||""}</p>
            <p class="question"><strong>Pregunta:</strong> ${t.question}</p>
        `),c){const d=F(t.question,t.options,t.correctIndex,u=>{J(u);const g=u===t.correctIndex;g&&i&&(p.inventory.includes(i.name)||(U(i.name),w(`¡ITEM OBTENIDO! "${i.name}"`))),w(`Nivel ${o+1}: respuesta ${g?"correcta":"incorrecta"}`)});d.addEventListener("panelFinished",()=>{setTimeout(()=>{o<E-1?te():T("end")},600)}),c.appendChild(d)}if(l){const d=$();p.log.forEach(u=>d.add(u)),l.appendChild(d.element)}if(v){const d=p.inventory,u=h.filter(m=>m.item&&d.includes(m.item.name)).map(m=>{const y=m.item.img;return{img:y?y.replace("../","/"):null,name:m.item.name}}).filter(m=>m.img!==null).sort((m,y)=>d.indexOf(m.name)-d.indexOf(y.name)),g=j(u);v.appendChild(g)}return e}const ce=Object.freeze(Object.defineProperty({__proto__:null,renderGame:A},Symbol.toStringTag,{value:"Module"}));function le(){const e=document.createElement("main"),n=document.createElement("div");return n.className="container",e.appendChild(n),e}function de(){const e=document.createElement("section");e.id="resultSection",e.setAttribute("aria-labelledby","result-content");const n=p.answers||[];let a=0;for(let m=0;m<E;m++){const y=h[m];n[m]===y.correctIndex&&a++}const o=E-1,t=n[o]===h[o].correctIndex,i=a>=3||t,s=n[0]===h[5].correctIndex,l=n[3]===h[3].correctIndex,r=n[5]===h[0].correctIndex,c=i||s&&l&&r,v=h[o],d=c?v.options[v.correctIndex].replace(/^[A-D]\.\s*/,""):"Desconocido",u=c?"Martillo":"Indeterminado",g=c?"Casa abandonada":"Indeterminado";return e.innerHTML=`
<div class="finalVerdict">
    ...
    `,e.querySelector("#goHome")?.addEventListener("click",m=>{m.preventDefault(),q("index.html")}),e}function ue(){const e=le(),n=e.querySelector(".container");if(!n)throw new Error("Container no encontrado");return n.appendChild(de()),e}function me(){const e=document.createElement("footer");return e.setAttribute("aria-label","Pie de página"),e.innerHTML=`
        ...
        >
  `,e}function pe(){return window.location.hash.slice(1)||"home"}function M(){const e=document.getElementById("app");if(!e)return;e.innerHTML="";const n=pe();let a;n==="game"?(a=A(),se()):n==="end"?(a=ue(),C()):(a=N(),C());const o=a.querySelector(".container");o&&o.appendChild(me()),e.appendChild(a)}document.addEventListener("DOMContentLoaded",M);window.addEventListener("hashchange",M);function T(e){if(e.includes(".html")||e.startsWith("/"))if(window.location.hostname.includes("github.io")){const a=window.location.pathname.split("/").filter(t=>t.length>0),o=a.length>0?a[0]:"";window.location.href=`/${o}/${e}`}else window.location.href=`/${e}`;else window.location.hash=e}
